...
async def handleHelloMessage(conn: "ConnectionHandler", msg_json):
    """处理hello消息"""
    conn.is_bridge = msg_json.get("bridge", False)
    conn.target_device = msg_json.get("target_device", None)
    if conn.is_bridge and hasattr(conn.server, 'connections'):
        conn.server.connections.pop(conn.device_id, None)
